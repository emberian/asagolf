//! sdgtanpure — the INTUITIONISTIC-PURITY GUARD.  The NEW trust root of the
//! sequel, mirroring the prequel's no-cheating / `--lint` guard.
//!
//! It mechanically certifies that NO classical principle appears (a) as
//! an $a in data/sdg.tangent.mm at all, nor (b) in the transitive consumed-$a
...
//!
//! Run:  cargo run --release --bin sdgtanpure
...
fn main() {
    let path = "data/sdg.tangent.mm";
    let src = std::fs::read_to_string(path).expect("read data/sdg.tangent.mm");
    let db = match kernel::Db::parse(&src) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("sdgtanpure: PARSE ERROR: {e}");
            std::process::exit(1);
        }
    };
    ...
    if let Err(e) = db.verify() {
        eprintln!("sdgtanpure: KERNEL REJECTED — refusing to certify: {e}");
        std::process::exit(1);
    }
    ...
    println!("=== sdgtanpure — INTUITIONISTIC-PURITY GUARD (new trust root) ===");
    println!(
