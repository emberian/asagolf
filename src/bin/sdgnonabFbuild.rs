//! sdgnonabFbuild — BOOK-3 WAVE-7: the bracket algebra underlying the
//! NON-ABELIAN homogeneous Yang–Mills field equation DF = ∇F + [A,F] = 0.
//! Writes data/sdg.nonabF.mm.
//!
//! HONEST FRAMING (stated up front, iron rule).  Over a COMMUTATIVE
//! base every symbolic bracket [x,y] := (x*y)+inv(y*x) is identically 0
//! (ax-mulcom + ax-addneg) — so a symbolic Jacobi identity would be
//! VACUOUS (0+0+0=0) and is NOT shipped as "the non-abelian engine".
//! The genuine non-abelian content lives ONLY in the structural 2×2
//! matrix route (waves 4–5, data/sdg.nonab*.mm) and its non-vacuity is
//! the GROUNDED cross-book spine 1≠0 (wave 6, data/sdg.spine.mm).  What
//! IS genuine, non-vacuous, and tractable here is the BRACKET ALGEBRA
//! that the non-abelian homogeneous field equation rests on — pure
//! inv/distribution identities, true in any ring, NOT commutator-
//! collapse:
//!
//!   * sdg-nonabF-neguniq  : ( ( p + q ) = 0 -> q = ( inv p ) )
//!       additive-inverse uniqueness (deduction form, pure ring)
//!   * sdg-nonabF-eqneg    : ( x = y -> ( inv x ) = ( inv y ) )
//!       the inv-congruence (deduction form; base has no eq-neg)
//!   * sdg-nonabF-invinv   : ( inv ( inv x ) ) = x          (involution)
//!   * sdg-nonabF-invadd   : ( inv ( u + v ) ) = ( ( inv u ) + ( inv v ) )
//!   * sdg-nonabF-mulneg   : ( a * ( inv w ) ) = ( inv ( a * w ) )
//!       scalar pulls through the additive inverse (left) — bracket
//!       bilinearity's engine
//!   * sdg-nonabF-mulnegr  : ( ( inv w ) * a ) = ( inv ( w * a ) )  (right)
//!   * sdg-nonabF-brktantisym :
//!       ( ( a * b ) + ( inv ( b * a ) ) )
//!         = ( inv ( ( b * a ) + ( inv ( a * b ) ) ) )
//!       the bracket is ANTISYMMETRIC: [a,b] = inv [b,a] (= −[b,a]) —
//!       the structural antisymmetry the homogeneous field equation
//!       DF = ∇F + [A,F] = 0 rests on, proven pure-ring via invadd +
//!       invinv (NOT via [.,.]=0 — true as an inv-distribution identity
//!       in ANY ring, commutative or not).
//!
//! WHAT THIS IS, precisely (no overclaim).  This is the pure-ring
//! BRACKET ALGEBRA of the non-abelian homogeneous field equation:
//! antisymmetry + scalar-bilinearity.  It is NOT the full DF=0
//! assembly, NOT the matrix-level Jacobi at a non-vanishing witness,
//! NOT the inhomogeneous (dynamical) Yang–Mills equation D⋆F=J, NOT an
//! action/Hodge/matter/quantization.  Those are the NAMED residuals
//! (BOOK3_SCOPE), the matrix-level non-vacuity being exactly the
//! already-GROUNDED spine 1≠0.  The model-meaning floor is the
...
//!
//! Run:  cargo run --release --bin sdgnonabFbuild
//! Trust root = src/kernel.rs re-checking data/sdg.nonabF.mm (also
//! sdgnonabFfloor / sdgnonabFpure).
#[path = "../kernel.rs"]
mod kernel;
...
fn main() {
    ...
    let sdg_nonabF_eqneg = imp_eqtr(&c, &imp_a1(&n7, &g_xy));
    ...
    // ===== sdg-nonabF-neguniq : ( ( u + v ) = 0 -> v = ( inv u ) ) =====
    let u = reg(&leaf("vu", "u"));
    let v = reg(&leaf("vv", "v"));
    let iu = neg(&u);
    let upv = pl(&u, &v);
    let g_uv = reg(&weq(&upv, &zero)); // G := ( u + v ) = 0
    // hyp-free links
    let q1 = eqsym(&add0(&v)); // v = ( v + 0 )
    let q2 = crp2(&eqsym(&addneg(&u)), &v); // ( v + 0 ) = ( v + ( u + inv u ) )
    let q3 = eqsym(&addass(&v, &u, &iu)); // = ( ( v + u ) + inv u )
    let q4 = clp1(&addcom(&v, &u), &iu); // ( ( v + u ) + iu ) = ( ( u + v ) + iu )
    // G-step: ( G -> ( ( u+v ) + iu ) = ( 0 + iu ) )
    let g_q5 = imp_cong_l(&idimp(&g_uv), &iu, "+", "tpl", "eq-pl1");
    let q6 = eqtr(&addcom(&zero, &iu), &add0(&iu)); // ( 0 + iu ) = inv u
    ...
    let sdg_nonabF_neguniq = imp_eqtr(&c, &imp_a1(&q6, &g_uv)); // ( G -> v = inv u )

    // neguniq rule: from P:( (s+t)=0 ) get  t = ( inv s ).
    ...
    // ===== sdg-nonabF-invinv : ( inv ( inv x ) ) = x ===================
    let ax2 = reg(&leaf("vx", "x"));
    let iax = neg(&ax2);
    let prem_ii = eqtr(&eqsym(&addcom(&ax2, &iax)), &addneg(&ax2)); // ( inv x + x ) = 0
    let x_eq_iix = neguniq(&prem_ii, &iax, &ax2); // x = inv ( inv x )
    let sdg_nonabF_invinv = eqsym(&x_eq_iix); // ( inv ( inv x ) ) = x

    // ===== sdg-nonabF-invadd : inv(u+v) = (inv u)+(inv v) =============
    let av = reg(&leaf("vv", "v"));
    let iav = neg(&av);
    let iu2 = neg(&u);
    let iuiv = pl(&iu2, &iav);
    // sub : ( v + ( iu + iv ) ) = inv u
    let sub = {
        let t_a = crp2(&addcom(&iu2, &iav), &av); // (v+(iu+iv))=(v+(iv+iu))
        let t_b = eqsym(&addass(&av, &iav, &iu2)); // =((v+iv)+iu)
        let t_c = clp1(&addneg(&av), &iu2); // ((v+iv)+iu)=(0+iu)
        let t_d = eqtr(&addcom(&zero, &iu2), &add0(&iu2)); // (0+iu)=inv u
        eqtr(&eqtr(&eqtr(&t_a, &t_b), &t_c), &t_d)
    };
    let prem_ia = {
        let s1 = addass(&u, &av, &iuiv); // ((u+v)+(iu+iv))=(u+(v+(iu+iv)))
        let s3 = crp2(&sub, &u); // (u+(v+(iu+iv)))=(u+inv u)
        let s4 = addneg(&u); // (u+inv u)=0
        eqtr(&eqtr(&s1, &s3), &s4) // ((u+v)+(iu+iv))=0
    };
    let uv2 = pl(&u, &av);
    let invadd_fwd = neguniq(&prem_ia, &uv2, &iuiv); // (iu+iv) = inv(u+v)
    let sdg_nonabF_invadd = eqsym(&invadd_fwd); // inv(u+v) = (iu+iv)

    let invadd_use = |uu: &W, vv: &W| -> Pf {
        use_thm(
            "sdg-nonabF-invadd",
            &[("u", uu), ("v", vv)],
            &[],
            reg(&weq(
                &neg(&reg(&binop(uu, vv, "+", "tpl"))),
                &reg(&binop(&neg(uu), &neg(vv), "+", "tpl")),
            ))
            .toks,
        )
    };
    ...
    let prem_mn = {
        ...
        eqtr(&eqtr(&m1, &m2), &m3) // (a*w)+(a*iw)=0
    };
    let aw = ml(&aa, &ww);
    let a_iw = ml(&aa, &iw);
    let sdg_nonabF_mulneg = neguniq(&prem_mn, &aw, &a_iw); // (a*iw)=inv(a*w)
    ...
    // ===== sdg-nonabF-mulnegr : ( ( inv w ) * a ) = ( inv ( w * a ) ) =
    let sdg_nonabF_mulnegr = {
        let r1 = mulcom(&iw, &aa); // (iw*a)=(a*iw)
        let r2 = mulneg_use(&aa, &ww); // (a*iw)=inv(a*w)
        let r3 = negcong(&mulcom(&aa, &ww)); // inv(a*w)=inv(w*a)
        eqtr(&eqtr(&r1, &r2), &r3)
    };

    // ===== sdg-nonabF-brktantisym :
    //   ( ( a * b ) + ( inv ( b * a ) ) ) = ( inv ( ( b * a ) + ( inv ( a * b ) ) ) )
    let bb = reg(&leaf("vb", "b"));
    let ab = ml(&aa, &bb);
    let ba = ml(&bb, &aa);
    let sdg_nonabF_brktantisym = {
        let lhs_k1 = addcom(&ab, &neg(&ba)); // ((a*b)+inv(b*a))=(inv(b*a)+(a*b))
        let k2 = crp2(&eqsym(&invinv_use(&ab)), &neg(&ba)); // (inv(b*a)+(a*b))=(inv(b*a)+inv(inv(a*b)))
        let k3 = eqsym(&invadd_use(&ba, &neg(&ab))); // (inv(b*a)+inv(inv(a*b)))=inv((b*a)+inv(a*b))
        eqtr(&eqtr(&lhs_k1, &k2), &k3)
    };
    ...
    match kernel::Db::parse(&out) {
        Ok(db) => match db.verify() {
            Ok(()) => {
                ...
                std::fs::write("data/sdg.nonabF.mm", &out).expect("write data/sdg.nonabF.mm");
                println!(
                    "sdgnonabFbuild: kernel-verified {n} $p; wrote data/sdg.nonabF.mm OK"
                );
            }
            Err(e) => {
                eprintln!("sdgnonabFbuild: KERNEL REJECTED (not written): {e}");
                std::fs::write("/tmp/sdg.nonabF.reject.mm", &out).ok();
                std::process::exit(1);
            }
        },
        Err(e) => {
            eprintln!("sdgnonabFbuild: PARSE ERROR (not written): {e}");
            std::fs::write("/tmp/sdg.nonabF.reject.mm", &out).ok();
            std::process::exit(1);
        }
    }
