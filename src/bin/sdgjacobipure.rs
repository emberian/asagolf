//! sdgjacobipure — intuitionistic-purity guard + MEASURED cost +
//! the STRONG adversarial assertion for data/sdg.jacobi.mm (BOOK-3
...
//!
//! Run:  cargo run --release --bin sdgjacobipure
...
fn main() {
    ...
    let db = match kernel::Db::parse(&src) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("sdgjacobipure: PARSE ERROR: {e}");
            std::process::exit(1);
        }
    };
    if let Err(e) = db.verify() {
        eprintln!("sdgjacobipure: KERNEL REJECTED — refusing to certify: {e}");
        std::process::exit(1);
    }
    ...
    println!("=== sdgjacobipure — NON-VACUOUS non-abelian JACOBI (Book-3 wave-8) purity + MEASURED ===");
    println!(
    ...
    if hard_fail {
        ...
        println!(
            ...
            \x20    no eq-ap.  The genuine non-vacuous non-abelian Jacobi\n\
            \x20    at the concrete witness (entrywise j11=j12=j21=j22=0,\n\
            \x20    the derived inv congruence) forces NO new substrate\n\
            \x20    commitment (§1b BOUND NOT triggered)."
        );
    }
    ...
    println!(
        "\nVERDICT: GENUINELY INTUITIONISTIC \u{2714} AND EVERY $p PURE RING.\n         \
         The genuine NON-VACUOUS non-abelian Jacobi at a concrete 2x2\n         \
         witness — A=[[0,1],[0,0]] B=[[0,0],[1,0]] C=[[1,0],[0,0]] over\n         \
         the FROZEN commutative ring, pairwise commutators genuinely\n         \
         NON-ZERO ([A,B]=[[1,0],[0,inv 1]] etc., NOT collapse), the\n         \
         cyclic Jacobi sum [A,[B,C]]+[B,[C,A]]+[C,[A,B]] vanishing\n         \
         ENTRYWISE (j11=j12=j21=j22=0), the derived inv congruence —\n         \
         closes with NO new substrate commitment, NO seam, NO eq-ap,\n         \
         nothing classical.  The recurring theorem holds even at the\n         \
         non-abelian Jacobi frontier.  THE CROSS-BOOK SPINE (named, not\n         \
         faked, not measured): the non-vacuity (that [A,B],[B,C],[C,A]\n         \
         are genuinely NON-ZERO) is EXACTLY 1!=0 — PRECISELY Book One's\n         \
         measured irreducible residue (suc0!=0 / orientation).  Book\n         \
         Three's non-abelian meaning-residue IS Book One's residue: the\n         \
         spine is one bone.  Reported, not faked."
    );
    std::process::exit(0);
}
