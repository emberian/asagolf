//! sdgnonabbuild — BOOK-3 WAVE-4: the NON-ABELIAN structure, over the
//! FROZEN base, NO new substrate commitment.  Writes data/sdg.nonab.mm.
//!
//! BOOK3_SCOPE §2 declared the commutative scalar-line reduction; the
//! full non-abelian content (F = dA + A∧A with every cross term, the
//! non-abelian commutator) was the one named residual.  This wave
//! attacks it the iron-rule way: NOT a new non-commutative axiom, but
//! 2×2 matrices over the FROZEN commutative ring R, where
//! non-commutativity emerges PURELY STRUCTURALLY from index contraction
//! (commuting entries, non-commuting product).  The canonical witness
//!     A = [[0,1],[0,0]]   B = [[0,0],[1,0]]
//! has, computed ENTRYWISE by pure ring algebra over the frozen base:
//!     A·B = [[1,0],[0,0]]   B·A = [[0,0],[0,1]]
//!     [A,B] = A·B − B·A = [[1,0],[0,(inv 1)]]
//! kernel-verified, NO new axiom, nothing classical.  This is GENUINE
//! non-abelian structure: [A,B] ≠ 0 structurally.
//!
//! THE CROSS-BOOK SPINE (the genuine new finding, named not faked).
//! That [A,B] is a genuinely NON-ZERO matrix — its non-vacuity — is
//! EXACTLY  1 ≠ 0 .  The frozen base does NOT assert 1 ≠ 0 (it is a
//! possibly-trivial ring; intuitionistic equality, no inequality
//! primitive).  And  1 ≠ 0  is PRECISELY Book One's measured
//! irreducible residue: the bottom of metric geometry was
//! { ∅∈suc∅ , suc∅≠∅ } ≅ 1≠0 / orientation.  So Book Three's
//! non-abelian meaning-residue IS Book One's residue — literally the
//! same  1≠0 .  The recurring theorem is not merely shaped the same
//! three times; the third book's irreducible meaning-cost reduces to
//! the first book's irreducible meaning-cost.  The spine is one bone.
//! This is REPORTED as a named [PROJECTION]-class residual, never
//! faked into the proof and never papered: the pure-ring commutator
//! identities are MEASURED; the non-vacuity 1≠0 is the named residual,
//! identical to Book One's COST_STRUCTURE bottom.
//!
//!   * sdg-nonab-eqneg : additive-inverse congruence
//!       ( x = y -> ( inv x ) = ( inv y ) ), the `inv` analog of
//!       sdg-addcan-imp, DERIVED pure-ring (deduction form) — NOT a
//!       base axiom (base has eq-pl/eq-mu/eq-ap only, no eq-neg).
//!   * sdg-nonab-comm11/12/21/22 : the four entries of [A,B] for the
//!       canonical witness — = 1 , 0 , 0 , ( inv 1 ).  PURE RING.
//!   * sdg-nonab-wedge : the non-abelian field-strength wedge cross-
//!       term IS the commutator: ( [A,B]₁₁·v )·( d*e ) = ( 1·v )·( d*e )
//!       — structurally NON-cancelling, in deliberate contrast to the
//!       abelian sdg-gauge-F-cross which DOES cancel (ax-mulcom).  This
//!       exhibits exactly where abelian and non-abelian gauge theory
//!       part ways, pure-ring, no new axiom.
//!
//! HONEST SCOPE.  What CLOSES: the non-abelian commutator + wedge,
//! PURE RING, kernel-verified, over the FROZEN base — NO new substrate
//! commitment (§1b BOUND is NOT triggered: the recurring theorem holds
//! even for genuine non-abelian structure).  Nothing classical.  What
//! REMAINS the named residual: the non-vacuity 1≠0 = Book One's exact
//! irreducible residue (the cross-book spine), and the FULL gauge-
//! field-equation assembly at general matrix size (the canonical 2×2
//! witness is the proof-of-concept, the Book-One-style concrete
//! separation, not the entire Yang–Mills tower) — both NAMED, not
//! papered.  The model-meaning floor is the UNCHANGED Book-Two
//! [PROJECTION], never re-summed.
//!
//! Composition / zero-conflict.  Self-contained over the IDENTICAL
//! FROZEN eq-ap-extended data/sdg.base.mm; disjoint `sdg-nonab-*`
//! labels; shares NO $p with any corpus — rename-free concatenation.
//! Modifies no kernel / base / other corpus / builder.
//!
//! Run:  cargo run --release --bin sdgnonabbuild
//! Trust root = src/kernel.rs re-checking the emitted file (also
//! sdgnonabfloor / sdgnonabpure).  Toolkit copied VERBATIM from
//! src/bin/sdgbuild.rs (deterministic constructors; kernel re-checks).
#[path = "../kernel.rs"]
mod kernel;
...
fn main() {
    ...
    let addcom =
        |a: &W, b: &W| axeq("ax-addcom", &[a, b], &reg(&binop(a, b, "+", "tpl")), &reg(&binop(b, a, "+", "tpl")));
    ...
    // ( inv 0 ) = 0   (pure ring; no inv-congruence needed yet)
    let inv0 = neg(&zero);
    let an0 = addneg(&zero); // ( 0 + ( inv 0 ) ) = 0
    let z_i0 = pl(&zero, &inv0);
    let i0_z = pl(&inv0, &zero);
    ...
    // =====================================================================
    //  sdg-nonab-eqneg : ( x = y -> ( inv x ) = ( inv y ) ) — the `inv`
    //  analog of additive cancellation, DERIVED pure-ring in deduction
    //  form (the base has eq-pl/eq-mu/eq-ap only; NO eq-neg axiom).
    // ---------------------------------------------------------------------
    let x = leaf("vx", "x");
    let y = leaf("vy", "y");
    let ix = neg(&x);
    let iy = neg(&y);
    let g_xy = reg(&weq(&x, &y)); // G := x = y

    // hyp-free links
    let e1 = eqsym(&add0(&ix)); // inv x = ( inv x + 0 )
    let e2 = crp2(&eqsym(&addneg(&y)), &ix); // ( inv x + 0 ) = ( inv x + ( y + inv y ) )
    let e3 = eqsym(&addass(&ix, &y, &iy)); // = ( ( inv x + y ) + inv y )
    let e5 = clp1(&addcom(&ix, &x), &iy); // ((inv x+x)+iy) = ((x+inv x)+iy)
    let e6 = clp1(&addneg(&x), &iy); // ((x+inv x)+iy) = (0+iy)
    let e7 = eqtr(&addcom(&zero, &iy), &add0(&iy)); // (0+iy) = inv y

    // the single G-using link, in deduction form
    let yx = reg(&weq(&y, &x));
    let p_yx = apply("eqcom", &[&x, &y], &[], reg(&wi(&g_xy, &yx)).toks); // (G -> y=x)
    let ixy = pl(&ix, &y);
    let ixx = pl(&ix, &x);
    ...
    let g_e4 = imp_cong_l(&g_sub, &iy, "+", "tpl", "eq-pl1"); // (G -> ((ix+y)+iy)=((ix+x)+iy))

    // chain all links under G
    ...
    let sdg_nonab_eqneg = imp_eqtr(&c, &imp_a1(&e7, &g_xy)); // |- ( x=y -> inv x = inv y )

    // negcong rule: from P:(a=b) get (inv a)=(inv b) via sdg-nonab-eqneg.
    let negcong = |p: &Pf| -> Pf {
        let (a, b) = split_eq(&p.stmt);
        let aw = w_of(&a);
        let bw = w_of(&b);
    ...
    // =====================================================================
    //  THE WITNESS  A=[[0,1],[0,0]]  B=[[0,0],[1,0]] :
    //    [A,B] = A·B − B·A = [[1,0],[0,(inv 1)]]   — entrywise PURE RING.
    // ---------------------------------------------------------------------
    let m00 = ml(&zero, &zero); // 0*0
    let m11 = ml(&one, &one); // 1*1
    let m10 = ml(&one, &zero); // 1*0
    let m01 = ml(&zero, &one); // 0*1
    let r00 = mul0(&zero); // 0*0 = 0
    let r11 = mul1(&one); // 1*1 = 1
    let r10 = mul0(&one); // 1*0 = 0
    let r01 = mul1(&zero); // 0*1 = 0
    let r0p0 = add0(&zero); // 0+0 = 0
    let r0p1 = eqtr(&addcom(&zero, &one), &add0(&one)); // 0+1 = 1

    // ( ( 0*0 ) + ( 0*0 ) ) = 0
    let s00_00 = pl(&m00, &m00);
    let i_dd = {
        let b1 = clp1(&r00, &m00); // (m00+m00)=(0+m00)
        let b2 = crp2(&r00, &zero); // (0+m00)=(0+0)
        eqtr(&eqtr(&b1, &b2), &r0p0) // = 0
    };
    let _ = &s00_00;
    let inv_s00_00 = neg(&pl(&m00, &m00));
    let inv_dd_eq0 = eqtr(&negcong(&i_dd), &pf_inv0); // ( inv ((0*0)+(0*0)) ) = 0

    // ---- comm11 : ( ( (0*0)+(1*1) ) + ( inv ((0*0)+(0*0)) ) ) = 1 ----
    let l11 = {
        let a1 = clp1(&r00, &m11); // ((0*0)+(1*1))=(0+(1*1))
        let a2 = crp2(&r11, &zero); // (0+(1*1))=(0+1)
        eqtr(&eqtr(&a1, &a2), &r0p1) // = 1
    };
    let comm11 = {
        let lhs_sum = pl(&m00, &m11);
    ...
    // ---- comm22 : ( ( (0*0)+(0*0) ) + ( inv ((1*1)+(0*0)) ) ) = ( inv 1 ) ----
    let i_11_00 = {
        let d1 = clp1(&r11, &m00); // ((1*1)+(0*0))=(1+(0*0))
        let d2 = crp2(&r00, &one); // (1+(0*0))=(1+0)
        eqtr(&eqtr(&d1, &d2), &add0(&one)) // = 1
    };
    let inv_11_00 = neg(&pl(&m11, &m00));
    let inv_1100_eq_inv1 = negcong(&i_11_00); // ( inv ((1*1)+(0*0)) ) = ( inv 1 )
    let comm22 = {
        let e1c = clp1(&i_dd, &inv_11_00); // (DD + inv1100)=(0 + inv1100)
        let e2c = crp2(&inv_1100_eq_inv1, &zero); // (0+inv1100)=(0+inv 1)
        let e3c = eqtr(&addcom(&zero, &neg(&one)), &add0(&neg(&one))); // (0+inv1)=inv 1
        eqtr(&eqtr(&e1c, &e2c), &e3c)
    };

    // ---- comm12 : ( ( (0*0)+(1*0) ) + ( inv ((0*1)+(0*0)) ) ) = 0 ----
    let lf12 = {
        let f1 = clp1(&r00, &m10); // ((0*0)+(1*0))=(0+(1*0))
        let f2 = crp2(&r10, &zero); // (0+(1*0))=(0+0)
        eqtr(&eqtr(&f1, &f2), &r0p0) // = 0
    };
    let i_01_00 = {
        let g1 = clp1(&r01, &m00); // ((0*1)+(0*0))=(0+(0*0))
        let g2 = crp2(&r00, &zero); // (0+(0*0))=(0+0)
        eqtr(&eqtr(&g1, &g2), &r0p0) // = 0
    };
    let inv_01_00 = neg(&pl(&m01, &m00));
    let inv_0100_eq0 = eqtr(&negcong(&i_01_00), &pf_inv0);
    let comm12 = {
        let h1 = clp1(&lf12, &inv_01_00); // (Lf+inv0100)=(0+inv0100)
        let h2 = crp2(&inv_0100_eq0, &zero); // (0+inv0100)=(0+0)
        eqtr(&eqtr(&h1, &h2), &r0p0)
    };

    // ---- comm21 : ( ( (0*0)+(0*1) ) + ( inv ((1*0)+(0*0)) ) ) = 0 ----
    let lf21 = {
        let f1 = clp1(&r00, &m01); // ((0*0)+(0*1))=(0+(0*1))
        let f2 = crp2(&r01, &zero); // (0+(0*1))=(0+0)
        eqtr(&eqtr(&f1, &f2), &r0p0) // = 0
    };
    let i_10_00 = {
        let g1 = clp1(&r10, &m00); // ((1*0)+(0*0))=(0+(0*0))
        let g2 = crp2(&r00, &zero); // (0+(0*0))=(0+0)
        eqtr(&eqtr(&g1, &g2), &r0p0) // = 0
    };
    let inv_10_00 = neg(&pl(&m10, &m00));
    let inv_1000_eq0 = eqtr(&negcong(&i_10_00), &pf_inv0);
    let comm21 = {
        let h1 = clp1(&lf21, &inv_10_00);
        let h2 = crp2(&inv_1000_eq0, &zero);
        eqtr(&eqtr(&h1, &h2), &r0p0)
    };

    // ---- sdg-nonab-wedge : ( [A,B]11 · v ) · ( d*e ) = ( 1·v )·( d*e ) -
    //  the non-abelian field-strength wedge cross-term IS the commutator
    //  — structurally NON-cancelling (contrast abelian sdg-gauge-F-cross,
    //  which cancels by ax-mulcom).  Built ON comm11.
    let vv = leaf("vv", "v");
    let dd = leaf("vd", "d");
    let ee = leaf("ve", "e");
    let de = ml(&dd, &ee); // ( d * e )
    let w1 = cong_l(&comm11, &vv, "*", "tmu", "eq-mu1"); // (C11*v)=(1*v)
    let sdg_nonab_wedge = cong_l(&w1, &de, "*", "tmu", "eq-mu1");

    let _ = (&comm11, &comm12, &comm21, &comm22, &sdg_nonab_wedge);
    ...
    let proofs: Vec<(&str, &str, Vec<(&str, &str)>, &Pf)> = vec![
        ...
        (
            "sdg-nonab-comm12",
            "|- ( ( ( 0 * 0 ) + ( 1 * 0 ) ) + ( inv ( ( 0 * 1 ) + ( 0 * 0 ) ) ) ) = 0",
            vec![],
            &comm12,
        ),
        (
            "sdg-nonab-comm21",
            "|- ( ( ( 0 * 0 ) + ( 0 * 1 ) ) + ( inv ( ( 1 * 0 ) + ( 0 * 0 ) ) ) ) = 0",
            vec![],
            &comm21,
        ),
        (
            "sdg-nonab-comm22",
            "|- ( ( ( 0 * 0 ) + ( 0 * 0 ) ) + ( inv ( ( 1 * 1 ) + ( 0 * 0 ) ) ) ) = ( inv 1 )",
            vec![],
            &comm22,
        ),
        (
            "sdg-nonab-wedge",
            "|- ( ( ( ( ( 0 * 0 ) + ( 1 * 1 ) ) + ( inv ( ( 0 * 0 ) + ( 0 * 0 ) ) ) ) * v ) * ( d * e ) ) = ( ( 1 * v ) * ( d * e ) )",
            vec![],
            &sdg_nonab_wedge,
        ),
    ];
    ...
    match kernel::Db::parse(&out) {
        Ok(db) => match db.verify() {
            Ok(()) => {
                ...
                std::fs::write("data/sdg.nonab.mm", &out).expect("write data/sdg.nonab.mm");
                println!(
                    "sdgnonabbuild: kernel-verified {n} $p; wrote data/sdg.nonab.mm OK"
                );
            }
            Err(e) => {
                eprintln!("sdgnonabbuild: KERNEL REJECTED (not written): {e}");
                std::fs::write("/tmp/sdg.nonab.reject.mm", &out).ok();
                std::process::exit(1);
            }
        },
        Err(e) => {
            eprintln!("sdgnonabbuild: PARSE ERROR (not written): {e}");
            std::fs::write("/tmp/sdg.nonab.reject.mm", &out).ok();
            std::process::exit(1);
        }
    }
