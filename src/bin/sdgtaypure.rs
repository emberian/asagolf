//! sdgtaypure — the INTUITIONISTIC-PURITY GUARD.  The NEW trust root of the
//! sequel, mirroring the prequel's no-cheating / `--lint` guard.
//!
//! It mechanically certifies that NO classical principle appears (a) as
//! an $a in data/sdg.taylor.mm at all, nor (b) in the transitive consumed-$a
...
//!
//! Run:  cargo run --release --bin sdgtaypure
...
fn main() {
    ...
    let db = match kernel::Db::parse(&src) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("sdgtaypure: PARSE ERROR: {e}");
            std::process::exit(1);
        }
    };
    ...
    if let Err(e) = db.verify() {
        eprintln!("sdgtaypure: KERNEL REJECTED — refusing to certify: {e}");
        std::process::exit(1);
    }
    ...
    println!("=== sdgtaypure — INTUITIONISTIC-PURITY GUARD (new trust root) ===");
    println!(
    ...
    // ---- ADVERSARIAL-HONESTY check (order-2 Taylor specific) -----------
    //  The seam-free headline sdg-tay-deriv2 must GENUINELY consume the
    //  level-2 uniqueness axiom ax-microcancel2 AND the level-2
    //  Kock-Lawvere structure (df-D2 — D_2 is what its hypotheses
    //  quantify over).  If it did NOT reach ax-microcancel2 the
    //  uniqueness would be hand-waved (a ZERO).  We report this
    //  explicitly so the trust story is auditable, not asserted.
    if let Some((_, ax)) = per_thm.iter().find(|(l, _)| l == "sdg-tay-deriv2") {
        let uses_mc2 = ax.contains("ax-microcancel2");
        println!(
            "\nADVERSARIAL-HONESTY (order-2 uniqueness must be genuine):\n  \
             sdg-tay-deriv2 consumes ax-microcancel2 : {}",
            if uses_mc2 { "YES ✔ (uniqueness genuinely threaded, not hand-waved)" }
            else { "NO ✗ — uniqueness is HAND-WAVED, this is a ZERO" }
        );
        if !uses_mc2 {
            eprintln!(
                "sdgtaypure: sdg-tay-deriv2 does NOT consume ax-microcancel2 — \
                 order-2 uniqueness is hand-waved.  REFUSING to certify (ZERO)."
            );
            std::process::exit(1);
        }
    }
