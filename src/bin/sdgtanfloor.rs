//! sdgtanfloor — kernel-verify data/sdg.tangent.mm (the POINTWISE synthetic
...
//!
//! Run:  cargo run --release --bin sdgtanfloor
//!
//! Every MEASURED line is produced here from OUR kernel over
//! data/sdg.tangent.mm.  The pointwise->global linking seam is the separate
...
fn main() {
    ...
    println!(
        "\n=== MEASURED in OUR kernel (data/sdg.tangent.mm), cut-free $a-leaf count ==="
    );
    for st in &db.stmts {
